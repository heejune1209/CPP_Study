...
public:
	...
	//		// 그러다 보니까 우리가 멋대로 이 펫을 아 이제는 내가 잘 쓸게 라고 예를 막 날려준다거나 상대방의 펫을 막 건드리고 이런 거는 당연히 말이 안된다
...
void TestKnight_LValueRef(Knight& knight) { } // 나이트 원본을 넘겨 줄테니 얘를 수정할수도있고 꺼내쓸수도 있고 원본을 멋대로 가지고 놀면돼
void TestKnight_ConstLValueRef(const Knight& knight) { /*knight.PrintInfo();  PrintInfo에 const를 붙여줘서 통과*/ } // 원본을 넘겨주긴 넘겨줄건데 원본을 수정하면 안되고 읽는거만 허락.
...
int main()
{
	...
	// const로 만들어주면 knight 이것과 관련된 정보들을 우리가 수정을 할수 없을 뿐더러 반드시 이제는 이런 식으로 const가 붙은 함수만 이제 또 호출할 수가 있다는 굉장히 불편한 상황이 된다
	...
	// k3를 오른값 참조로 캐스팅한 후에 k4로 이동시킨다음 k3의 펫 소유권을 박탈시키는 기능을 하게 될 것이다. (이동 대입 연산자 코드 참고)
