...
// 물론 매개변수에 참조가 잘못된 메모리를 가리키는 경우 포인터와 똑같이 크래시가 나는 등의 위험해질수 있다
...
// 물론 C++에서 참조는 초기화 시 안전한 메모리 주소에 바인딩되도록 설계되었지만, 다음과 같은 경우에는 참조도 포인터처럼 위험해질 수 있습니다 :
// 1. 참조가 잘못된 메모리(삭제된 객체, nullptr)를 가리키는 경우.
// 2. 비정상적인 강제 캐스팅으로 참조를 초기화한 경우.
// 따라서, 참조를 사용할 때도 항상 유효한 객체나 메모리 주소를 참조하도록 보장해야 하며, 포인터와 마찬가지로 주의를 기울여야 합니다.
...
// 그러니까 결론적으로 이런 경우가 위험하다.
// 1. 함수 인자에 잘못된 메모리나 변수를 전달한 경우
// - 참조와 포인터 모두 유효하지 않은 메모리를 참조하거나 NULL 포인터처럼 잘못된 값을 전달받으면 **크래시나 Undefined Behavior(UB)**가 발생할 수 있습니다.
// 2. 함수 스택 내에 생성된 변수를 반환값으로 반환한 경우
// - 함수 내부에서 선언된 로컬 변수는 함수 종료 시 스택에서 소멸됩니다.
// - 참조나 포인터를 통해 이 소멸된 메모리를 접근하려 하면 Undefined Behavior가 발생합니다.
// 3. 함수 스택 내에 생성한 포인터나 참조에 매개변수를 넣어서 래핑한다음 반환하는것도 해당 메모리가 소멸되기 때문에 위험하다. 
